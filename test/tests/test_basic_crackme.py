...
def test_next():
    ...
    dbg.next() # step over the call to printf, which is the next instruction, and
...
def test_single_step():
    ...
    dbg.continue_execution()
    assert dbg.registers.rip == main + 0x44
    dbg.single_step() # step into the call to printf, which is the next instruction, and should take us into the plt stub for printf
...
def test_memory_read_write():
    ...
    read_num = dbg.read_number(read_num_address, Int64)
    assert read_num == 123
    # write a new number and check it
    dbg.write_number(read_num_address, CRACKME_SOL, 8)
    read_num_after_write = dbg.read_number(read_num_address, Int64)
