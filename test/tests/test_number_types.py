...
sys.set_int_max_str_digits(
    0
)  # some numbers are too big to display in strings for pytest
...
def test_integer_behavior(cls, num1, num2):
    ...
    assert int(a * b) == c_wrap(num1 * num2, bits, signed)
    if num2 != 0 and (
        num2 != -1 or num1 != -(2 ** (bits - 1))
    ):  # avoid division overflow case
    ...
    assert int(abs(-a)) == c_wrap(
        abs(c_wrap(-num1, bits, signed)), bits, signed
    )  # we need to do this as negation can cause a wrap and if the number is the minimum negative abs can cause a wrap
    assert int(abs(b)) == c_wrap(abs(num2), bits, signed)
    assert int(abs(-b)) == c_wrap(
        abs(c_wrap(-num2, bits, signed)), bits, signed
    )  # we need to do this as negation can cause a wrap and if the number is the minimum negative abs can cause a wrap
...
@pytest.mark.parametrize(
    "cls_higher, cls_lower, val1, val2",
    [
...
def test_priority(cls_higher, cls_lower, val1, val2):
    ...
    assert a + b == a + cls_higher(
        b
    )  # check that it doesn't overflow based on lower class
...
def test_different_type_comparison():
    ...
    assert Int16(-1) > UInt16(
        1
    )  # as UInt16 has higher priority, this would evaluate to 0xffff > 1 which is true
    assert Int32(-1) < UInt16(
        1
    )  # this time Int32 has higher priority, so this would evaluate to -1 < 1 which is true
...
def test_float80_precision():
    ...
    """
    base_float = 1.0
    small_increment_float = 1e-18
    base = Float80(base_float)
    small_increment = Float80(small_increment_float)
    ...
    # verify that float indeed isn't precise enough for this
    a_float = base_float + small_increment_float
    b_float = base_float + small_increment_float * 2
    diff_float = b_float - a_float
    assert diff_float == pytest.approx(0.0, abs=tolerance)
...
def test_shift_wrapping():
    ...
    assert int(Int8(4) >> 8) == 0
    assert int(Int16(4) >> 16) == 0
    assert int(Int32(4) >> 33) == 2
    assert int(Int64(4) >> 66) == 1
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            float("inf"),
            add,
        ),  # too big to fit in float, we need to use an int
        (Float32, -2e38, -2e38, float("-inf"), add),
        (Float64, -1e308, -1e308, float("-inf"), add),
...
def test_special_float_result(cls, val1, val2, result, operator):
    ...
    assert not math.isnan(
        float(a)
    )  # as nan != nan, for nan test we convert to float and check with math.isnan
