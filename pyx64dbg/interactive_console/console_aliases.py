...
ALL_ALIASES: list[tuple[list[str], tuple[list[str | list[str]] | object, list[str | list[str]] | object], str | None]] = [
    ...
    (["finish", "fin", "f"], (["debugger", "control", "finish"], ["_process_not_running_trap"]), "Step out of the current function."),
...
def get_aliases(console: "InteractiveConsole", process_running: bool) -> dict[str, object]:
    ...
    """
    res: dict[str, object] = {}
    for names, (active_path, inactive_path), _ in ALL_ALIASES:
...
def get_all_commands_help() -> list[tuple[list[str], str]]:
    """
    Returns a list of (aliases, help_description) for all commands that have a help description (i.e. where the description isn't None).
    This is used for the help message in the interactive console.
    """
    res : list[tuple[list[str], str]] = []
    for names, _, help_desc in ALL_ALIASES:
...
def get_all_command_names() -> list[str]:
    ...
    """
    names: list[str] = []
    for aliases, _, _ in ALL_ALIASES:
