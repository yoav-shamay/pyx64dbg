...
def _c_mod(num1, num2):
    # C modulus is negative if num1 is negative, while python doesn't
    if num2 == 0:
...
class CInt(Generic[CT]):
    ...
    is_signed: bool = (
        None  # Indicates whether the type is signed, should be set by subclasses
    )
    ...
    def _choose_result_type(self, other):
        if isinstance(other, CInt):
            return (
                self.__class__ if self.priority >= other.priority else other.__class__
            )
    ...
    def __iadd__(self, other):
        ...
        if rhs is NotImplemented:
            return NotImplemented
        return self.__class__(self.num.value + rhs) # in case of in place modification, we want to return same type (not promote)
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}({self.num.value},{hex(self.num.value)})"
    ...
    def from_bytes(cls, bytes_data: bytes, byteorder="little"):
        int_value = int.from_bytes(
            bytes_data, byteorder=byteorder, signed=cls.is_signed
        )
        return cls(int_value)

    def to_bytes(self, byteorder="little"):
        return self.num.value.to_bytes(self.size, byteorder=byteorder, signed=self.is_signed)
